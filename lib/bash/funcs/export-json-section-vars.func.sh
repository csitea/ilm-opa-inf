...
do_export_json_section_vars() {
  ...
  do_log "INFO exporting vars from section: $section json_file: $json_file: "
  while read -r l; do
  ...
  # ok cat dev.env.json | jq -r '.env.steps."004-aws-iam"|to_entries| map(select(.value | type == "string"))|from_entries|keys_unsorted[] as $key|"\($key):\"\(.[$key])\""'
}
