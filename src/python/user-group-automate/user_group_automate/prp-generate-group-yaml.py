...
path_to_prp_yaml = Path("/opt/spe/spe-infra-conf/prp/all/006-individual-users/users.yaml").expanduser()
with open(path_to_prp_yaml, "r") as file:
    prp_user_group_yaml = yaml.safe_load(file)
...
csi_wpb_workbook = pd.read_excel(path_to_excel, csi_wpb_sheet_names, true_values=["x"])
...
def generate_role_list_for_user(sheet_name, sheet_user_email):
    ...
    for user_column_value in csi_wpb_workbook[sheet_name].loc[csi_wpb_workbook[sheet_name]["EmailAddress"] == sheet_user_email].iloc[0].items():
...
prp_yaml_email_list=[]
for obj in prp_user_group_yaml:
    prp_yaml_email_list.append(obj["username"])

# List for the output YAML
prp_user_group_yaml_updated = []

# Add old yaml with empty groups to output YAML
for email in prp_yaml_email_list:
    prp_user_group_yaml_updated.append({"username": email, "groups": []})

# Iterate through sheets
for sheet_name in csi_wpb_sheet_names:
    print(f"Working on sheet: {sheet_name}")

    # Get list of all emails in sheet under work
    sheet_email_list = csi_wpb_workbook[sheet_name]["EmailAddress"].to_list()
    ...
    for sheet_user_email in sheet_email_list:
        ...
        if sheet_user_email not in [d["username"] for d in prp_user_group_yaml_updated]:
            prp_user_group_yaml_updated.append({"username": sheet_user_email, "groups": []})

        # Iterate through users in updated YAML
        for user in prp_user_group_yaml_updated:
...
# Write to file
path_to_write = Path("/opt/spe/spe-infra-conf/prp/all/006-individual-users/users-new.yaml").expanduser()
with open(path_to_write, 'w') as file:
    documents = yaml.dump(prp_user_group_yaml_updated, file)
